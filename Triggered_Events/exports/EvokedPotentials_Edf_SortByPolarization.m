...
if findobj('type','figure')       %is there a figure?
    response = questdlg('Close all plots?','User Input Required','Yes','No','Yes');  %if so, ask to close all plots with a UI
    if strcmp(response,'Yes')  %If the answer is yes
...
'Select EDF File','MultiSelect','On');  %one can select multiple edf files at a time.
if ~iscell(files)
    if isequal(files,0)
        return;
    end
