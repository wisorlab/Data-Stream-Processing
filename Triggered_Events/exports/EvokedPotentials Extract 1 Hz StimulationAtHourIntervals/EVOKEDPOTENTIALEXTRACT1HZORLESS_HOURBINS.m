...
prompt = { 'How many msec before trigger onset should each snippet start?' };
msbefore = inputdlg(prompt,'Input',1,{'500'});
msbefore = str2double( msbefore{1,1} );

prompt = { 'How many msec after trigger onset should each snippet end?' };
msafter = inputdlg(prompt,'Input',1,{'500'});
msafter = str2double( msafter{1,1} );
...
for i = 1:length(files)
    ...
    [waves,randoms] = findTriggerPattern1Hzorless_Hourly(data, matrix(:,ttlIndex), fs, msbefore, msafter, sleepdata);  %removed from outputs list: ,stimFreqs,usedTriggers
    ...
    for j = 1:length(waves(1,:,1))
        ...
        if exportState == 1
            ...
            xl.setCells( sheet, [ 1, rowCount(j) ], columnInfo );
            ...
            WaveDump = cell(1,size(waves(1,j,:),3));
            RandDump = cell(1,size(randoms(1,j,:),3));
            
            for HourCount=1:length(waves(:,1,1))
                xl.setCells( sheet, [8+(HourCount-1)*(length(waves(1,1,:))*2+1)+2 ,1], timeRowLabel );  %xl.setCells inserts the data into a given Excel Sheet , identity of which is specified here in the variable 'sheets'.
                %[8,1] specifies the cell (eighth column across to preserve identifer columns A-G; 1 puts the column lablels in the top row
                WaveDump(1:size(waves(HourCount,j,:),3)) = waves(HourCount,j,:);
                RandDump(1:size(randoms(HourCount,j,:),3)) = randoms(HourCount,j,:);
                
                xl.setCells( sheet, [8+(HourCount-1)*(length(waves(1,1,:))*2+1)+2, rowCount(j)], WaveDump ); %[8, rowCount(j)
                xl.setCells( sheet, [8+(HourCount-1)*(length(waves(1,1,:))*2+1)+(length(waves(1,1,:)))+2, rowCount(j)], RandDump );
            end %HourCount loop that counts through all hours of file.
            
            sleepstateCount = sleepstateCount + 1;
        end
        if plotState == 1
            label = [type,' at ', num2str(stimFreqs(ceil(j/4))),' Hz during ', sleepstate,' in ',files{i}];
            fig = plotStimulationPattern3(t,WaveDump,RandDump,fs,units,label,triggerPoint,matrix(:,1),data,matrix(:,ttlIndex),usedTriggers{j},sleepdata);
        end %if statement
    end %j loop that counts through the four states.
