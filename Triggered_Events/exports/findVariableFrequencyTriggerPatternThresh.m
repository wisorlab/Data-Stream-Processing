function [avgWave, randoms, stimulationFrequencies, usedTriggers, numOfWavesInState] = findVariableFrequencyTriggerPatternThresh(eeg, ttl, fs, msBefore, msAfter, sleepdata)
...
avgWave = zeros(length(firstPulseForStimFrequency)*4*3,samplesPerWave);
numOfWavesInState = zeros(length(firstPulseForStimFrequency)*4*3,2);
randoms = zeros(size(avgWave));
usedTriggers = cell(length(firstPulseForStimFrequency)*4*3,1);
numOfRandomWavesInState = zeros(length(firstPulseForStimFrequency)*4*3,2);
...
%     triggerStartsOfInterest = triggerStartsOfInterest(nonArtifacts);
    ...
    for j = 1:length(sampleEnds)
        ...
        if eeg(sampleStarts(j)+sampleStart) > threshold
            polarizationState = 0;
        elseif eeg(sampleStarts(j)+sampleStart) < threshold && eeg(sampleStarts(j)+sampleStart) > -threshold
            polarizationState = 1;
        elseif eeg(sampleStarts(j)+sampleStart) < -threshold
            polarizationState = 2;
        end
        avgWave(state+4*polarizationState+12*(i-2),1:samplesPerWave) = avgWave(state+4*polarizationState+12*(i-2),1:samplesPerWave)+eeg(sampleStarts(j):sampleEnds(j))';
        numOfWavesInState(state+4*polarizationState+12*(i-2),1) = numOfWavesInState(state+4*polarizationState+12*(i-2))+1;
        numOfWavesInState(state+4*polarizationState+12*(i-2),2) = find(sampleStarts(i) >= (firstPulseForStimFrequency-sampleStart),1,'last');
        usedTriggers{state+4*polarizationState+12*(i-2)} = [usedTriggers{state+4*(i-2)}, triggerStartsOfInterest(j)];
        ...
        if eeg(startIndex+sampleStart) > threshold
            polarizationState = 0;
        elseif eeg(startIndex+sampleStart) < threshold && eeg(startIndex+sampleStart) > -threshold
            polarizationState = 1;
        elseif eeg(startIndex+sampleStart) < -threshold
            polarizationState = 2;
        end
        randoms(state+4*polarizationState+12*(i-2),1:samplesPerWave) = randoms(state+4*polarizationState+12*(i-2),1:samplesPerWave)+eeg(startIndex:startIndex+samplesPerWave-1)';
        numOfRandomWavesInState(state+4*polarizationState+12*(i-2),1) = numOfRandomWavesInState(state+4*polarizationState+12*(i-2))+1;
        numOfRandomWavesInState(state+4*polarizationState+12*(i-2),2) = find(sampleStarts(i) >= (firstPulseForStimFrequency-sampleStart),1,'last');
    end
end
