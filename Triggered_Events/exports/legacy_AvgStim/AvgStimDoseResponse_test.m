...
for i = 1:length(files)
     ...
     if exportState == 1
        % if ~isempty(strfind(lower(name),'opto'))
        %     stimType = 'Opto';
        ...
        % if ~isempty(strfind(lower(name),'2cm')) || ~isempty(strfind(lower(name),'2 cm'))
        %     intensity = '2cm';
        % elseif ~isempty(strfind(lower(name),'4cm')) || ~isempty(strfind(lower(name),'4 cm'))
        %     intensity = '4cm';
        % elseif ~isempty(strfind(lower(name),'6cm')) || ~isempty(strfind(lower(name),'6 cm'))
        %     intensity = '6cm';
        % elseif ~isempty(strfind(lower(name),'8cm')) || ~isempty(strfind(lower(name),'8 cm'))
        %     intensity = '8cm';
        % elseif ~isempty(strfind(lower(name),'250 mv')) || ~isempty(strfind(lower(name),'250mv'))
        %     intensity = '250mV';
        % elseif ~isempty(strfind(lower(name),'500 mv')) || ~isempty(strfind(lower(name),'500mv'))
        %     intensity = '500mV';
        % elseif ~isempty(strfind(lower(name),'750 mv')) || ~isempty(strfind(lower(name),'750mv'))
        %     intensity = '750mV';
        % elseif ~isempty(strfind(lower(name),'1000 mv'))  || ~isempty(strfind(lower(name),'1000mv')) || ~isempty(strfind(lower(name),'1.0 v'))
        %     intensity = '1000mV';
        % end 

        result = regexp(name,'(\d+-\d+-\d+)\s(\w+)\s+((\d+.\d+|\d+)\s+\w+)\s+(\d+\s+\w+)','tokens');
        [timestamp stimType duration frequency ] = result{1}{:};
