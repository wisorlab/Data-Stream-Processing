...
Threshold = str2double( Threshold{1,1} );
...
%      end
     ...
     for j = 1:length(waves(:,1))
        ...
        if exportState == 1
            ...
            xl.setCells( sheets{j}, [1,1], columnLabels ); 
            xl.setCells( sheets{j}, [1,rowCount(j)],columnInfo);
...
%             randoms100 = avg4(randoms);
            ...
            xl.setCells( sheets{j}, [8,1], timeRowLabel );
            xl.setCells( sheets{j}, [8, rowCount(j)], waves(j,:) );
            xl.setCells( sheets{j}, [length(waves(j,:))+9, rowCount(j)], randoms(j,:) );
