function [avgWave, randoms, ttlonsets] = findTriggerPattern1Hzorless_Hourly(eeg, ttl, fs, msBefore, msAfter, sleepdata)
...
TotalHours=floor(length(eeg)/(fs*3600))+1;
avgWave = zeros(TotalHours,4,samplesPerWave); %make a vector of zeros in preparation for generating an avg wave
numOfWavesInState = zeros(TotalHours,4);  % four rows, two columns. This is an event counter for # waves each state. Each row= a state; not clear what the second column is there for.
randoms = zeros(size(avgWave)); %make a vector of zeros in preparation for generating an avg random curve
numOfRandomWavesInState = zeros(TotalHours,4);   % four rows, two columns. This is an event counter for # randoms each state. Each row= a state; not clear what the second column is there for.
...
for i = 1:length(sampleEnds)   %go through all detected data snippets one by one.
    ...
    HourNum=floor(sampleStarts(i)/(fs*3600))+1;
    z(1,1,1:samplesPerWave) = eeg(sampleStarts(i):sampleEnds(i));
    avgWave(HourNum,state,1:samplesPerWave) = avgWave(HourNum,state,1:samplesPerWave)+ z(1,1,1:samplesPerWave);
    %+eeg(sampleStarts(i):sampleEnds(i))';  %adds the eeg values to the appropriate row of the average wave matrix, based on state.
    numOfWavesInState(HourNum,state) = numOfWavesInState(HourNum,state)+1;   %count the number of waves in rows, each corresponding to a unique state.
    ...
    z(1,1,1:samplesPerWave) = eeg(startIndex:startIndex+samplesPerWave-1);
    randoms(HourNum,state,1:samplesPerWave) = randoms(HourNum,state,1:samplesPerWave)+z(1,1,1:samplesPerWave); %adds the eeg values to the appropriate row of the average wave matrix, based on state.
    numOfRandomWavesInState(HourNum,state) = numOfRandomWavesInState(HourNum,state)+1;
end


for HourCount=1:TotalHours
    for i = 1:length(avgWave(HourCount,:,1))
        if ~isempty(find(avgWave(HourCount,i,:), 1))
            avgWave(HourCount,i,:) = avgWave(HourCount,i,:)./numOfWavesInState(HourCount,i);
        end
        if ~isempty(find(randoms(HourCount,i,:), 1))
            randoms(HourCount,i,:) = randoms(HourCount,i,:)./numOfRandomWavesInState(HourCount,i);
        end %loop for state dimension of avgWave
    end %loop for hour dimension of avgWave
end

for HourCount=1:TotalHours
    for statecount=1:length(avgWave(HourCount,:,1))
        if max(avgWave(HourCount,statecount,:))==0 &&  min(avgWave(HourCount,statecount,:))==0
            avgWave(HourCount,statecount,:)=nan;
            randoms(HourCount,statecount,:)=nan;
        end
    end
end
end
