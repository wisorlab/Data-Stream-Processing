function [hdr,data] = mcdRead(filename,path)
...
if nargin < 1
    [filename,path] = uigetfile({'*.mcd','Multi Channel Systems Files (*.mcd)';'*.*','All Files';},... % Open the user interface for opening files
    'Select Data File');
    if ~iscell(filename)
...
if strcmp(reshape(fread(fid,29,'*char'),1,29),'MC_DataTool binary conversion');
    ...
    hdr.channels = splitstring(strtrim(header(strfind(lower(header),'streams =')+9:strfind(lower(header),'eoh')-1)),';');
    data = fread(fid,'int16');
    data = reshape(data,numel(hdr.channels),length(data)/numel(hdr.channels));
    data = data';
    ElColumns = find(not(cellfun('isempty', strfind(hdr.channels, 'El'))));
    DiColumns = find(not(cellfun('isempty', strfind(hdr.channels, 'Di'))));
    if hdr.ElFactor ~= 0
