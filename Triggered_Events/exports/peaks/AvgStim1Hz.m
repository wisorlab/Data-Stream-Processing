% Output routine for Thy1 animals stimulated at 1Hz only.
...
%      end
     ...
     [waves,randoms, usedTriggers] = findTriggerPattern1HzContinuous(data, matrix(:,ttlIndex), fs, msbefore, msafter, sleepdata);
...
%             randoms100 = avg4(randoms);
        ...
        if plotState == 1
            label = [type,' at 1 Hz in ',files{i}];
            fig = plotStimulationPattern3(label,matrix(:,1),data,matrix(:,ttlIndex),sleepdata);
        end
     end
